    ...
    <html lang="vi">
      <body className="min-h-screen bg-white text-[var(--text)]">
        <header className="sticky top-0 z-20 border-b bg-white/85 backdrop-blur">
          <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-6">
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold tracking-tight">TourQuote · Sơn Đông</span>
            </Link>
            <nav className="flex items-center gap-1 text-sm">
              ...
              <Link href="/quotes/new"
                className="ml-1 rounded-md bg-[var(--text)] px-3 py-1.5 font-medium text-white transition hover:opacity-90">
                + Báo giá
              </Link>
        ...
        <footer className="mt-20 border-t py-8 text-center text-xs text-[var(--text-muted)]">
          TourQuote · Công cụ báo giá nội bộ · Dữ liệu tour Sơn Đông
        </footer>
      </body>
    </html>
  );
